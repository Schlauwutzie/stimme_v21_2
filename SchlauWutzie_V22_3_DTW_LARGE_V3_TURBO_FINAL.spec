...
asset_intro = project / "assets" / "SchlauWutzie_AI_Datacenter_Intro_COUNTDOWN_FINAL.mp4"
runtime = project / "whisper_runtime"
model = project / "whisper_models" / "ggml-large-v3-turbo-q5_0.bin"
...
for item in [
    ...
    if not item.exists():
        raise SystemExit(
            f"FEHLER: V22.3 Build-Asset fehlt: {item}"
        )

ff_datas, ff_binaries, ff_hidden = collect_all(
    "imageio_ffmpeg"
)
...
exe = EXE(
    ...
    analysis.datas,
    [],
    name="SchlauWutzie_V22_3_DTW_LARGE_V3_TURBO_FINAL",
    debug=False,
