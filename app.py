...
APP_NAME = "SchlauWutzie K.I. – Video Studio V22 FINAL"
IN_W, IN_H = 720, 1280
...
# ---------------------------------------------------------------------------
# Deutsche Auto-Untertitel – V22 / whisper.cpp
# ---------------------------------------------------------------------------
...
def _parse_whisper_timestamp(value: str) -> float:
    value = str(value or "").strip()
    parts = value.replace(".", ",").split(":")
    if len(parts) != 3:
        raise ValueError(f"Ungültiger whisper.cpp Zeitstempel: {value!r}")
...
def _group_words_for_caption(
    ...
    """
    Create natural TikTok-sized caption groups from real Whisper word
    timestamps. Punctuation stays attached to the preceding word.
    """
    groups = []
    current = []
    ...
    for raw in words:
        token = _escape_ass_text(raw[0]).strip()
        if not token:
            continue

        start = float(raw[1])
        end = max(start + 0.04, float(raw[2]))

        if token in punctuation and current:
            current[-1] = (
                current[-1][0] + token,
                current[-1][1],
                max(current[-1][2], end),
...
def _find_whisper_cli() -> Path:
    ...
    if not cli.exists():
        # Useful for a developer checkout outside the EXE.
        local = Path(__file__).resolve().parent / "whisper_runtime" / "whisper-cli.exe"
        if local.exists():
            cli = local

    if not cli.exists():
        raise RuntimeError(
            "whisper-cli.exe fehlt.\n\n"
            "Die V22 EXE wurde ohne das eingebettete whisper.cpp Runtime-Paket "
            "gestartet."
        )
...
def _whisper_model_path() -> Path:
    ...
    if not model.exists():
        raise RuntimeError(
            "Das V22 Whisper-Sprachmodell fehlt:\n\n"
            + str(model)
        )
    return model
...
def _normalize_prompt(text: str) -> str:
    ...
    # whisper.cpp accepts an initial prompt; keep it bounded so the model's
...
def transcribe_german_to_ass(
    ...
    """
    Use whisper.cpp v1.9.1 with the multilingual small-q5_1 model.

    V22 intentionally uses an external, dependency-light C++ executable:
    no Python ML runtime, no CTranslate2, no System.Speech and no first-run
    model download.

    - language forced to German
    - one processor to avoid timestamp boundary issues
    - --max-len 1 for word-level timestamp segments
    - user's pasted text is supplied as an initial prompt to improve names,
      numbers and wording recognition
    """
    source = Path(audio_path)
    if not source.exists():
        raise RuntimeError(
            "Die Audio-Datei für die Untertitel wurde nicht gefunden."
        )
    ...
    temp_dir = Path(
        tempfile.mkdtemp(
            prefix="schlawutzie_whispercpp_"
        )
    )
    output_base = temp_dir / "result"
    json_path = Path(
        str(output_base) + ".json"
    )

    if status_callback:
        status_callback(
            "V22: whisper.cpp wird gestartet …"
        )

    prompt = _normalize_prompt(source_text)
    ...
    try:
        ...
        raise RuntimeError(
            "V22: Die deutsche Spracherkennung wurde nach "
        ...
        raise RuntimeError(
            "V22: whisper-cli.exe konnte nicht gestartet werden.\n\n"
            + str(exc)
        ) from exc
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(
            "V22: whisper.cpp konnte die deutsche Audiospur nicht verarbeiten.\n\n"
            + (
                stderr[-4000:]
                if stderr
    ...
    if not json_path.exists():
        ...
        raise RuntimeError(
            "V22: whisper.cpp hat keine Ergebnisdatei erzeugt.\n\n"
    ...
    try:
        ...
        for item in transcription:
            ...
            try:
                start = _parse_whisper_timestamp(
                    timestamps.get("from")
                )
                end = _parse_whisper_timestamp(
                    timestamps.get("to")
                )
            except Exception:
                continue

            # --max-len 1 normally gives exactly the needed word-sized
            # segments; punctuation may appear separately and is grouped later.
            raw_words.append(
                (
                    text,
                    start,
        ...
        raise RuntimeError(
            "V22: Die whisper.cpp JSON-Ausgabe konnte nicht gelesen werden.\n\n"
            + str(exc)
        ) from exc
    ...
    if not raw_words:
        raise RuntimeError(
            "V22: Es wurden keine deutschen Wörter erkannt."
        )
    ...
    ass_dir = Path(
        tempfile.mkdtemp(
            prefix="schlawutzie_v22_subtitles_"
        )
    )
    ...
    for group in groups:
        ...
        # Karaoke tags use each real word's own duration, so the highlight
        # tracks the recognized audio timing instead of guessing from frames.
        parts = []
        ...
        ass_lines.append(
            f"Dialogue: 0,{_ass_time(group_start)},"
            f"{_ass_time(group_end)},TikTok,"
            f",0,0,390,,{text}"
        )
    ...
    if status_callback:
        status_callback(
            f"V22: {len(groups)} deutsche Untertitelblöcke synchron erzeugt."
        )
...
# ---------------------------------------------------------------------------
# Deutsche Auto-Untertitel – isolierter V22 Hilfsprozess
# ---------------------------------------------------------------------------
...
class App(tk.Tk):
    ...
    def _subtitle_done(self, path):
        ...
        try:
            ...
            self._subtitle_error(
                "Die erzeugte Untertiteldatei konnte nicht dauerhaft "
                "gespeichert werden.\n\n"
                + str(exc)
            )
            return
