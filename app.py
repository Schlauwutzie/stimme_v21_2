...
APP_NAME = "SchlauWutzie K.I. – Video Studio V22.5 EXTERNAL MODEL FINAL"
IN_W, IN_H = 720, 1280
...
# ---------------------------------------------------------------------------
# V22.5 Deutsche Auto-Untertitel – Faster-Whisper / CapCut-style timing
# ---------------------------------------------------------------------------

def _v25_ass_time(seconds: float) -> str:
...
def _v25_escape_ass(text: str) -> str:
    return (
        str(text or "")
        .replace("\r", " ")
        .replace("\n", " ")
...
def _v25_has_repetition(words):
    """
    Detect obvious decoder collapse such as:
    Erfurter Erfurter Erfurter Erfurter ...
    """
    tokens = [
        re.sub(
            r"[^\wÄÖÜäöüß-]",
    ...
    # Also catch repeated 2-word loops.
...
def _v25_transcribe_faster_whisper(
    audio_path: str,
    source_text: str = "",
    status_callback=None,
):
    ...
    - Silero VAD
    - condition_on_previous_text=False to reduce long-form repetition loops
    - fallback temperatures
    - repetition penalty + no-repeat n-grams
    - hallucination silence threshold
    """
    from faster_whisper import WhisperModel

    # V22.5 FIX: Keep the 1.62 GB model OUTSIDE the EXE.
    # In a PyInstaller one-file EXE, bundling the model forces a large
    # extraction step and can make model loading fail. The model now lives
    # beside the EXE under models/faster-whisper-large-v3-turbo.
    exe_dir = Path(
        sys.executable
    ).resolve().parent if getattr(
        sys, "frozen", False
    ) else Path(__file__).resolve().parent

    model_dir = exe_dir / "models" / "faster-whisper-large-v3-turbo"
    ...
    if not model_dir.exists():
        raise RuntimeError(
            "V22.5: Das Faster-Whisper-Modell fehlt:\n\n"
            + str(model_dir)
        )

    if status_callback:
        status_callback(
            "V22.5: Schnellsprecherkennung wird geladen …"
        )
    ...
    try:
        ...
        raise RuntimeError(
            "V22.5: Faster-Whisper konnte das lokale Modell nicht laden.\n\n"
            f"Modellpfad:\n{model_dir}\n\n"
    ...
    segments, info = run_once()

    if status_callback:
        status_callback(
            "V22.5: Wort-Zeitstempel werden ausgewertet …"
        )
    ...
    for segment in segments:
        ...
        if seg_words:
            ...
            # Safe segment fallback if a word timestamp is unavailable.
            text = str(
                getattr(
                    segment,
                    "text",
            ...
            for i, (part, weight) in enumerate(
                zip(parts, weights)
            ):
                if i == len(parts) - 1:
                    part_end = seg_end
    ...
    # Ensure strict forward chronology.
    cleaned = []
    previous_end = 0.0
    ...
    # If the decoder visibly collapsed into a repetition loop, run one clean
    # second pass without the script prompt and with stronger repetition
    # suppression. Faster-Whisper documents condition_on_previous_text=False
    # specifically as a way to reduce repetition-loop failures.
    if _v25_has_repetition(cleaned):
        if status_callback:
            status_callback(
                "V22.5: Wiederholungsfehler erkannt – zweite Erkennung läuft …"
            )
...
def transcribe_german_to_ass(
    ...
    work = Path(
        tempfile.mkdtemp(
            prefix="schlawutzie_v25_faster_whisper_"
        )
    )

    try:
        words = _v25_transcribe_faster_whisper(
            str(source),
        ...
        if not words:
            raise RuntimeError(
                "V22.5: Es wurden keine verwertbaren deutschen Wörter erkannt."
            )

        groups = _v25_group_words(
            words,
        )

        ass_dir = Path(
            tempfile.mkdtemp(
                prefix="schlawutzie_v25_ass_"
            )
        )
        ...
        lines = [
            ...
            # Yellow spoken word + white upcoming words.
        ...
        for group in groups:
            ...
            for word, word_start, word_end in group:
                duration_cs = max(
                    1,
                    int(
                        round(
            ...
            lines.append(
                "Dialogue: 0,"
                + _v25_ass_time(group_start)
                + ","
                + _v25_ass_time(group_end)
                + ",TikTok,,0,0,850,,"
                + " ".join(parts)
            )
        ...
        if status_callback:
            status_callback(
                f"V22.5: {len(groups)} deutsche Untertitelblöcke synchron erzeugt."
            )
...
# ---------------------------------------------------------------------------
# V22.5 Faster-Whisper subprocess helper
# ---------------------------------------------------------------------------

def _v25_subtitle_helper_entry(
    audio_path: str,
    output_ass: str,
    status_path: str,
    prompt_path: str,
    ...
    try:
        ...
        _status_write(
            Path(status_path),
            "V22.5: Deutsche Auto-Untertitel fertig.",
        )
