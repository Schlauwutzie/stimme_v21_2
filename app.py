...
APP_NAME = "SchlauWutzie K.I. – Video Studio V22.4 CAPCUT STYLE FINAL"
IN_W, IN_H = 720, 1280
...
def _v23_words_from_full_json(payload):
    """
    CapCut-like timing path:
    use whisper.cpp's own word-sized segment timestamps.

    The CLI is run with --max-len 1 + --split-on-word. whisper.cpp then
    wraps the transcription into short word-sized segments and keeps a
    proper segment start/end range. We use those ranges directly instead
    of reconstructing words from DTW token internals.

    If a returned segment still contains more than one word, its duration
    is divided proportionally by word length as a safe fallback.
    """
    words = []
    last_end = 0.0
    ...
    for seg in payload.get("transcription", []):
        ...
        try:
            seg_start = _v22_parse_ts(ts.get("from"))
            seg_end = _v22_parse_ts(ts.get("to"))
        except Exception:
            continue
        ...
        # Split punctuation away only for measurement. It is attached back
        # to the final lexical word.
        parts = re.findall(
            r"\S+",
            text,
            flags=re.UNICODE,
        ...
        if len(parts) == 1:
            ...
            words.append(
                (
                    token,
                    seg_start,
                    max(seg_start + 0.05, seg_end),
                )
            )
            last_end = seg_end
            continue

        # Fallback for a segment containing multiple words:
        # allocate its measured duration by visible character weight.
        weights = [
            max(
                1.0,
                len(re.sub(r"[^\wÄÖÜäöüß-]", "", part)),
            )
            for part in parts
        ]
        total = sum(weights)
        ...
        for index, (part, weight) in enumerate(
            zip(parts, weights)
        ):
    ...
    # Remove impossible duplicate/overlapping spans while preserving order.
    cleaned = []
    prev_end = 0.0
...
def transcribe_german_to_ass(
    ...
    if status_callback:
        status_callback(
            "V22.4: deutsche Wörter werden mit echten Wort-Zeitfenstern synchronisiert …"
        )
    ...
    try:
        ...
        raise RuntimeError(
            "V22.4: Die Sprachsynchronisierung wurde nach "
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(
            "V22.4: Whisper konnte das Audio nicht verarbeiten.\n\n"
            + (
                stderr[-4000:]
    ...
    if not json_path.exists():
        ...
        raise RuntimeError(
            "V22.4: Whisper hat keine vollständige Zeitstempel-Datei erzeugt."
        )
    ...
    try:
        ...
        raise RuntimeError(
            "V22.4: Die Wort-Zeitstempel konnten nicht gelesen werden.\n\n"
            + str(exc)
        ) from exc
    ...
    if not words:
        raise RuntimeError(
            "V22.4: Es wurden keine verwertbaren Wort-Zeitstempel erkannt."
        )
