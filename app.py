...
APP_NAME = "SchlauWutzie K.I. – Video Studio V22.6 FINAL"
IN_W, IN_H = 720, 1280
...
INTRO_VIDEO = resource_path("assets/SchlauWutzie_NEW_INTRO_COUNTDOWN_FINAL.mp4")
...
# ---------------------------------------------------------------------------
# V22.6 FINAL – Deutsche Auto-Untertitel (Faster-Whisper)

def _final_ass_time(seconds: float) -> str:
...
def _final_ass_escape(text: str) -> str:
    return (
        str(text or "")
        .replace("\\", "")
        .replace("\r", " ")
...
def _final_group_words(words, max_words=7, max_chars=40, max_duration=2.4):
    groups, current = [], []
    ...
    for word, start, end in words:
        ...
        if not current:
            ...
            continue
        candidate = (" ".join(x[0] for x in current) + " " + word).strip()
        if (
            len(current) >= max_words
            or len(candidate) > max_chars
            or end - current[0][1] > max_duration
...
def _final_has_repetition_loop(words, run_length=4):
    tokens = [
        re.sub(r"[^\wÄÖÜäöüß-]", "", str(w).lower())
        for w, _, _ in words
    ]
    tokens = [t for t in tokens if t]
    if len(tokens) < run_length:
        return False
    run = 1
...
def _final_model_dir() -> Path:
    exe_dir = (
        Path(sys.executable).resolve().parent
        if getattr(sys, "frozen", False)
        else Path(__file__).resolve().parent
    )
    return exe_dir / "models" / "faster-whisper-large-v3-turbo"


def _final_transcribe_words(audio_path: str, status_callback=None):
    try:
        from faster_whisper import WhisperModel
    except Exception as exc:
        raise RuntimeError(
            "V22.6: Faster-Whisper konnte nicht geladen werden.\n\n"
            f"{type(exc).__name__}: {exc}"
        ) from exc

    model_dir = _final_model_dir()
    required = ["config.json", "model.bin", "preprocessor_config.json", "tokenizer.json"]
    missing = [x for x in required if not (model_dir / x).exists()]
    if missing:
        raise RuntimeError(
            "V22.6: KI-Sprachmodell fehlt.\n\n"
            f"Erwarteter Ordner:\n{model_dir}\n\n"
            "Fehlende Dateien:\n" + "\n".join(missing)
        )

    if status_callback:
        status_callback("V22.6: KI-Sprachmodell wird geladen …")
    ...
    try:
        ...
        raise RuntimeError(
            "V22.6: KI-Sprachmodell konnte nicht geladen werden.\n\n"
            f"Pfad:\n{model_dir}\n\n"
            f"{type(exc).__name__}: {exc}"
        ) from exc
    ...
    def run(pass_no: int):
        if status_callback:
            status_callback(
                f"V22.6: Deutsche Sprache wird analysiert (Durchlauf {pass_no}/2) …"
            )
        ...
        segments, _ = model.transcribe(
            ...
            best_of=5,
            patience=1.0,
            repetition_penalty=1.08 if pass_no == 1 else 1.12,
            no_repeat_ngram_size=3 if pass_no == 1 else 4,
            temperature=[0.0, 0.2, 0.4, 0.6, 0.8],
            compression_ratio_threshold=2.4 if pass_no == 1 else 2.0,
            log_prob_threshold=-1.0 if pass_no == 1 else -0.7,
            ...
            hallucination_silence_threshold=1.0 if pass_no == 1 else 1.2,
            max_new_tokens=448,
            chunk_length=30,
            log_progress=False,
        )
        ...
        for segment in segments:
            for item in (getattr(segment, "words", None) or []):
                ...
                if not text or start is None or end is None:
                    continue
                start = max(prev_end, float(start))
                end = max(start + 0.05, float(end))
    ...
    words = run(1)
    if _final_has_repetition_loop(words):
        if status_callback:
            status_callback("V22.6: Wiederholungsfehler erkannt – zweite KI-Analyse …")
        words = run(2)

    if not words:
        raise RuntimeError("V22.6: Es wurden keine deutschen Wörter erkannt.")
    if _final_has_repetition_loop(words):
        raise RuntimeError(
            "V22.6: Die Spracherkennung erzeugt erneut eine Wiederholungsschleife. "
            "Untertitel wurden aus Sicherheitsgründen nicht erzeugt."
        )
    return words


def transcribe_german_to_ass(audio_path: str, status_callback=None, source_text: str = "") -> str:
    words = _final_transcribe_words(audio_path, status_callback=status_callback)
    groups = _final_group_words(words)

    ass_dir = Path(tempfile.mkdtemp(prefix="schlawutzie_final_ass_"))
    ...
    lines = [
        ...
        "Style: TikTok,Segoe UI,52,&H0000FFFF,&H00FFFFFF,&H00000000,&H99000000,-1,0,0,0,100,100,0,0,3,2,1,2,80,80,850,1",
        "",
        "[Events]",
    ...
    for group in groups:
        start_t = group[0][1]
        end_t = max(group[-1][2], start_t + 0.25)
        karaoke = []
        for word, ws, we in group:
            cs = max(1, int(round((we - ws) * 100)))
            karaoke.append(r"{\k" + str(cs) + "}" + word)
        lines.append(
            "Dialogue: 0,"
            + _final_ass_time(start_t)
            + ","
            + _final_ass_time(end_t)
            + ",TikTok,,0,0,850,,"
            + " ".join(karaoke)
        )
    ...
    if status_callback:
        status_callback(f"V22.6: {len(groups)} synchronisierte Untertitelblöcke erzeugt.")
    return str(ass_path)


def _v22_subtitle_helper_entry(audio_path: str, output_ass: str, status_path: str, prompt_path: str):
    try:
        def status_callback(message):
            _status_write(Path(status_path), message)
        ass_path = transcribe_german_to_ass(audio_path, status_callback=status_callback, source_text="")
        shutil.copy2(ass_path, output_ass)
        _status_write(Path(status_path), "V22.6: Deutsche Auto-Untertitel fertig.")
        return 0
    except Exception as exc:
        _status_write(Path(status_path), "FEHLER: " + f"{type(exc).__name__}: {exc}")
        return 1


if __name__ == "__main__" and len(sys.argv) == 6 and sys.argv[1] == "--v22-subtitle-helper":
    raise SystemExit(_v22_subtitle_helper_entry(sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5]))
...
def burn_ass_subtitles(
    ...
    """
    Burn the generated ASS subtitles into the main video.

    This function is deliberately kept separate from recognition. It uses the
    same FFmpeg subtitle burn-in approach that worked in the earlier stable
    versions, and it preserves the original audio stream.
    """
    ff = ffmpeg_path()
    ...
    if result.returncode != 0:
        raise RuntimeError(
            "V22.6: Untertitel konnten nicht in die MP4 eingebrannt werden.\n\n"
            + result.stderr.decode(errors="ignore")[-5000:]
        )
