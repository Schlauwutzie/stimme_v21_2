...
APP_NAME = "SchlauWutzie K.I. – Video Studio V22.3 FINAL"
IN_W, IN_H = 720, 1280
...
def _v23_words_from_full_json(payload):
    """
    Convert whisper.cpp --output-json-full token data into continuous word
    spans for the COMPLETE audio.

    DTW (`t_dtw`) is a token START timestamp.  It does not contain an end
    timestamp.  The previous implementation set `end = start`, which could
    collapse later caption timing.  Here the end of a word is taken from the
    next word's DTW start when available, otherwise from the normal token
    timestamp/segment end.

    The parser also falls back to normal `timestamps.from/to` whenever DTW
    data are missing or non-monotonic, so a later segment cannot disappear.
    """
    words = []
    current_parts = []
    current_start = None
    current_end = None
    last_global_start = 0.0
    last_segment_end = 0.0
    last_token_text = ""

    def flush(end_override=None):
        ...
        if text and current_start is not None:
                ...
                else float(current_end or current_start)
            )
            end = max(
    ...
    def token_times(tok, seg_end):
        ts = tok.get("timestamps", {}) or {}
        try:
            normal_start = _v22_parse_ts(ts.get("from"))
            normal_end = _v22_parse_ts(ts.get("to"))
        except Exception:
            normal_start = None
            normal_end = None

        dtw = tok.get("t_dtw", -1)
        if isinstance(dtw, (int, float)) and float(dtw) >= 0:
            candidate = float(dtw) / 100.0
        else:
            candidate = None

        # DTW is a START time. Accept it only when it moves forward. Otherwise
        # use the normal token timestamp, which is still absolute per segment.
        if candidate is not None and candidate + 0.001 >= last_global_start:
            start = candidate
        elif normal_start is not None:
            start = normal_start
        else:
            start = last_global_start

        if normal_end is not None:
            end = max(start + 0.02, normal_end)
        else:
            end = max(start + 0.02, float(seg_end))
    ...
    for seg in payload.get("transcription", []):
        ...
        try:
            ...
            seg_end = last_segment_end
        last_segment_end = max(last_segment_end, seg_end)

        for tok in seg.get("tokens", []) or []:
            if not isinstance(tok, dict):
                continue

            raw_text = str(tok.get("text", "") or "")
            if not raw_text:
                continue

            # Ignore Whisper control/timestamp tokens.
            if raw_text.startswith("[_") or raw_text.startswith("<|"):
                continue

            start, token_end = token_times(tok, seg_end)

            # Keep the global cursor monotonic across segments.
            if start < last_global_start:
                start = last_global_start
                token_end = max(token_end, start + 0.02)

            clean = raw_text.replace("\u2581", " ")
            starts_new_word = bool(clean[:1].isspace()) and bool(current_parts)
            ...
            if starts_new_word:
                # The next word's DTW start is the most reliable end of the
                # previous word.  Never create a zero/negative span.
                flush(end_override=max(
                    float(start),
                    float(current_end or start),
                ))

            if current_start is None:
                current_start = start

            # Preserve exact token text; punctuation remains attached.
            if not current_parts:
                current_parts.append(clean.lstrip())
            else:
                current_parts.append(clean)

            current_end = max(
                float(current_end or token_end),
                float(token_end),
            )
            last_global_start = max(
                last_global_start,
                float(start),
            )
            last_token_text = stripped

            # Punctuation closes the current word immediately. It will still
            # keep the following word's exact DTW start as its final boundary.
            if is_punctuation:
                # Do not flush yet if the punctuation token itself contains
                # other text; the next word boundary will provide the precise
                # end. Just remember the end.
                current_end = max(current_end, token_end)

    flush(
        end_override=max(
            float(current_end or last_segment_end),
            float(last_segment_end),
        )
        if current_start is not None
        else None
    )

    # Remove impossible/duplicate spans and enforce strictly forward timing.
    clean_words = []
    prev_end = 0.0
    for text, start, end in words:
        ...
        if text:
            clean_words.append((text, start, end))
            prev_end = end
...
def transcribe_german_to_ass(
    ...
    lines = [
        ...
        "-1,0,0,0,100,100,0,0,3,2,1,2,80,80,850,1",
        "",
        "[Events]",
    ...
    if status_callback:
        status_callback(
            f"V22.3: {len(groups)} synchronisierte Untertitelblöcke über die komplette Sprachspur erzeugt."
        )
